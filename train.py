...
def cosine_scheduler(lr, min_lr, epochs, steps_per_epoch, warmup_epochs=0,
    ...
    if cur_epoch is None:
        ...
        return np.array([min_lr + 0.5 * (lr - min_lr) * (1 + math.cos(math.pi * (i - warmup_iters) / (len(iters)))) for i in np.arange(cur_epoch*steps_per_epoch, cur_epoch*steps_per_epoch+steps_per_epoch)])    
...
def main():
    ...
    # set output
    out_dir = opt.output_dir + '_m' + str(opt.layer_num_m) + 'p' + str(opt.layer_num_p) \
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        if dist_value < best_test_dist:
            logger.log(f"Reach best test performance, save checkpoint to {ckpt_dir}")
            best_test_dist = dist_value
            saved = True
            torch.save(estimator.state_dict(), os.path.join(ckpt_dir, f"epoch_{epoch}_dist_{dist_value}.pth"))
